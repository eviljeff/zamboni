...
def check_addon_ownership(request, addon, viewer=False, dev=False,
    ...
    if addon.is_deleted:
        return False
    # Users with 'Addons|Apps|Personas:Edit' can do anything.
    if admin:
        if addon.type == amo.ADDON_WEBAPP:
            ...
            only = 'addon'
        if check_reviewer(request=request, only=only, senior=True):
            return True
...
def check_reviewer(request, only=None, senior=False):
    perm = 'Edit' if senior else 'Review'
    addon = action_allowed(request, 'Addons', perm)
    app = action_allowed(request, 'Apps', perm)
    persona = action_allowed(request, 'Personas', perm)
...
def check_snr_reviewer(request, only):
    return check_reviewer(request=request,only=only,senior=True)
