...
def app_view(f, qs=Webapp.with_deleted.all):
    @functools.wraps(f)
    def wrapper(request, app_slug=None, *args, **kw):
        """Provides an addon object (app) given an app_slug."""
        assert app_slug, 'Must provide app_slug'
