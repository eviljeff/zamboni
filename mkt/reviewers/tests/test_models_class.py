...
class TestReviewerScore(mkt.site.tests.TestCase):
    ...
    def setUpTestData(cls):
        cls.user = user_factory(username='editor')
        cls.editor_group = Group.objects.create(
            name='App Reviewers', rules='Apps:Review')
        GroupUser.objects.create(group=cls.editor_group, user=cls.user)
        cls.admin_user = user_factory(username='admin')
        cls.admin_group = Group.objects.create(
            name='Admins', rules='*:*')
        GroupUser.objects.create(group=cls.admin_group, user=cls.admin_user)
