...
class TestModerateLog(AppReviewerTest, AccessMixin):
...
class TestModerateLogDetail(TestModerateLog):

    def test_detail_page(self):
        mkt.log(mkt.LOG.APPROVE_REVIEW, self.review.addon, self.review)
        id = ActivityLog.objects.editor_events()[0].id
        r = self.client.get(reverse('reviewers.apps.moderatelog.detail',
                                    args=[id]))
        eq_(r.status_code, 200)

    def test_undelete_selfmoderation(self):
        id = mkt.log(mkt.LOG.DELETE_REVIEW, self.review.addon, self.review).id
        self.review.delete()
        url = reverse('reviewers.apps.moderatelog.detail', args=[id])
    ...
    def test_undelete_unauthorized(self):
        ...
        id = mkt.log(mkt.LOG.DELETE_REVIEW, self.review.addon, self.review,
